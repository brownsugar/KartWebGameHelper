(() => {
  const EVENT_KEY = 'kwgh-korea-2019-0911'
  ...
  kwgh.ev(EVENT_KEY, 'jewelry', 'count', parseInt(kwgh.el.q('.all_point').innerText))

  const remainCoinEl = kwgh.el.q('.use_point')
  const finishRoundEl = kwgh.el.q('.com_cnt')
  ...
  function doPlay() {
    ...
    playRequest().then(data => {
      ...
      const coupon = {
        id: data.n4ItemNo,
        name: GetItemData(data.n4ItemNo, 'name'),
        sn: data.strCouponSN
      }
